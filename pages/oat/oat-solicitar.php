...
<body>
	...
	<div class="container">
		...
		if(isset($_POST['cadastrar'])):
			...
			$oat->setNome($nome);
			$oat->setEmail($email);
			$oat->setNickuser($nickuser);
			$oat->setSenha($senha);
			$oat->setSexo($sexo);
			$oat->setRaca($raca);
			$oat->setNascimento($nascimento);
			$oat->setDataCadastro($datacadastro);
			$oat->setDatalogin($datalogin);
			$oat->setNivel($nivel_oat);
			$oat->setAtivodo($ativado);
			# Insert
			if($oat->insert()){
